...
**A-parameter quirk you must know.** In the code `Ann = amp * N_COINS` ([`curve-contract/contracts/pools/3pool/StableSwap3Pool.vy:202`](curve-contract/contracts/pools/3pool/StableSwap3Pool.vy#L202), [`stableswap-ng/contracts/main/CurveStableSwapNG.vy:1099`](stableswap-ng/contracts/main/CurveStableSwapNG.vy#L1099)), i.e. the contract's `A` already absorbs a factor `n^(n-1)` relative to the whitepaper's `A`. The `A` you read on-chain (3pool: 2000) is the contract value. In NG the stored value is additionally multiplied by `A_PRECISION = 100` ([`CurveStableSwapNG.vy:176`](stableswap-ng/contracts/main/CurveStableSwapNG.vy#L176)) so `A()` returns `_A()/100` (`:1775`) and `A_precise()` returns the raw value (`:1781`).
...
* `range(255)`: Vyper requires a compile-time loop bound; 255 is a generous cap, not a real expectation. The classic code *returns* whatever it has after 255 iterations; NG `raise`s instead ([`CurveStableSwapNG.vy:1127`](stableswap-ng/contracts/main/CurveStableSwapNG.vy#L1127)).
...
`CurveStableSwapNGMath.vy` (269 lines) exposes `get_y` (`:18`), `get_D` (`:90`), `get_y_D` (`:143`), `exp` (`:203`) as `@external @pure` with `N_COINS` as an argument. The plain pool inlines its own copies ([`CurveStableSwapNG.vy:1009-1184`](stableswap-ng/contracts/main/CurveStableSwapNG.vy#L1009-L1184)) because internal calls are cheaper; the **metapool** instead calls `math.get_D(...)` / `math.get_y(...)` ([`CurveStableSwapMetaNG.vy:1117-1118`](stableswap-ng/contracts/main/CurveStableSwapMetaNG.vy#L1117-L1118), `math` immutable at `:216`) to stay under the 24 KB contract-size limit — metapools carry the extra `exchange_underlying` logic.

`CurveStableSwapNGViews.vy` (704 lines) is a stateless helper deployed once per chain and registered in the factory (`set_views_implementation`, [`CurveStableSwapFactoryNG.vy:813`](stableswap-ng/contracts/main/CurveStableSwapFactoryNG.vy#L813)). The pool's `get_dy` ([`CurveStableSwapNG.vy:1702`](stableswap-ng/contracts/main/CurveStableSwapNG.vy#L1702)), `get_dx` (`:1688`), `calc_token_amount` (`:1760`), `dynamic_fee` (`:1811`) are one-liners that forward to `StableSwapViews(factory.views_implementation()).x(..., self)`. The Views contract re-reads `stored_rates()` / `get_balances()` / `A()` / `fee()` / `offpeg_fee_multiplier()` from the pool (`_get_rates_balances_xp`, `Views:690-704`) and re-runs the maths (`Views:59-88` for `get_dy`, including the dynamic fee at `:82`). Benefits: pool bytecode stays small; quoting logic (e.g. `get_dx`, `:349-378` — inverting the swap by solving `get_y(j, i, y_target)` with the fee grossed-up) can be upgraded by the DAO without touching pools; integrators get `calc_token_amount` **with** fees (`:395-492`) unlike classic.
...
1. **Reproduce the trace in §1.11** in Python: port `get_D` ([`curve-contract/contracts/pools/3pool/StableSwap3Pool.vy:195-219`](curve-contract/contracts/pools/3pool/StableSwap3Pool.vy#L195-L219)) and `get_y` (`:356-398`) verbatim with integer division, then confirm `dy = 999,422,408` for the given state. Then change `A` to 200 and 20000 and watch `dy` move.
...
4. **Dynamic fee**: in [`stableswap-ng/contracts/main/CurveStableSwapNG.vy:887-901`](stableswap-ng/contracts/main/CurveStableSwapNG.vy#L887-L901), compute `fee_dyn / fee` for `m = 5` at balance ratios 1:1, 2:1, 10:1, 100:1. Then find where the *same* function is applied in `add_liquidity` (`:634-642`) and explain what `xs` and `ys` represent there.
...
6. **Metapool**: trace `exchange_underlying(1, 3, …)` (DAI → USDT) through [`CurveStableSwapMetaNG.vy:659-751`](stableswap-ng/contracts/main/CurveStableSwapMetaNG.vy#L659-L751) and `_transfer_in` (`:423-489`). Which of the three cases in §2.5 is it, and how many external calls does it make?
...
8. **Gauge**: in [`curve-dao-contracts/contracts/gauges/LiquidityGauge.vy:153-220`](curve-dao-contracts/contracts/gauges/LiquidityGauge.vy#L153-L220), explain what happens to a user who deposited and never touched the gauge for 3 years (hint: the `range(500)` loop, `prev_future_epoch` handling at `:190-196`, and the comment "the gauge gets less"). Then find the equivalent in [`stableswap-ng/contracts/main/LiquidityGauge.vy:225-296`](stableswap-ng/contracts/main/LiquidityGauge.vy#L225-L296) and list what V6 changed.

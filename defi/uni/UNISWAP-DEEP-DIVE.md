...
- `UniswapV2ERC20` (`v2-core/contracts/UniswapV2ERC20.sol`): the LP token, with EIP-2612 `permit` at [`UniswapV2ERC20.sol:81`](v2-core/contracts/UniswapV2ERC20.sol#L81).
...
The core/periphery split is deliberate: the pair is minimal and "low-level; should be called from a contract which performs important safety checks" (comment at [`UniswapV2Pair.sol:109`](v2-core/contracts/UniswapV2Pair.sol#L109)). Slippage checks, deadlines, and token pulling live in the router.

### 1.2 `Factory.createPair` and deterministic addresses

[`v2-core/contracts/UniswapV2Factory.sol:23-38`](v2-core/contracts/UniswapV2Factory.sol#L23-L38)
...
4. **External calls**: `create2` with `salt = keccak256(token0, token1)` (`:29-32`), then `pair.initialize(token0, token1)` (`:33`) which only the factory may call ([`UniswapV2Pair.sol:66-70`](v2-core/contracts/UniswapV2Pair.sol#L66-L70)).
5. **Emits** `PairCreated`.

Because the salt is only the sorted token addresses and the creation code is fixed, the pair address is a pure function of `(factory, token0, token1)`. The periphery exploits this in `UniswapV2Library.pairFor` ([`UniswapV2Library.sol:18-26`](v2-periphery/contracts/libraries/UniswapV2Library.sol#L18-L26)):
...
[`v2-core/contracts/UniswapV2Pair.sol:15-36`](v2-core/contracts/UniswapV2Pair.sol#L15-L36)
...
[`v2-core/contracts/UniswapV2Pair.sol:110-131`](v2-core/contracts/UniswapV2Pair.sol#L110-L131)
...
[`v2-core/contracts/UniswapV2Pair.sol:89-107`](v2-core/contracts/UniswapV2Pair.sol#L89-L107)
...
[`v2-core/contracts/UniswapV2Pair.sol:134-156`](v2-core/contracts/UniswapV2Pair.sol#L134-L156)
...
[`v2-core/contracts/UniswapV2Pair.sol:159-187`](v2-core/contracts/UniswapV2Pair.sol#L159-L187)
...
[`v2-core/contracts/UniswapV2Pair.sol:73-86`](v2-core/contracts/UniswapV2Pair.sol#L73-L86)
...
- Prices are `UQ112x112` fixed-point ([`libraries/UQ112x112.sol:8-19`](v2-core/contracts/libraries/UQ112x112.sol#L8-L19)): a 112-bit integer part and 112-bit fraction, so `encode(a).uqdiv(b) = (a << 112) / b`.
...
**Amount math** ([`UniswapV2Library.sol:43-59`](v2-periphery/contracts/libraries/UniswapV2Library.sol#L43-L59)):
...
So `P = 2000` is tick `76012` (`ln 2000 / ln 1.0001 = 76012.8`), and `P = 1/2000` is tick `-76012`. `TickMath.MIN_TICK = -887272`, `MAX_TICK = 887272` ([`v3-core/contracts/libraries/TickMath.sol:9-11`](v3-core/contracts/libraries/TickMath.sol#L9-L11)), giving prices from `2^-128` to `2^128`. Pools only allow positions on ticks that are multiples of `tickSpacing` (10, 60 or 200 depending on the fee tier, [`UniswapV3Factory.sol:26-30`](v3-core/contracts/UniswapV3Factory.sol#L26-L30)).
...
**Factory and deployer.** `UniswapV3Factory.createPool` ([`v3-core/contracts/UniswapV3Factory.sol:35-51`](v3-core/contracts/UniswapV3Factory.sol#L35-L51)) sorts tokens, looks up `tickSpacing` for the fee (`:43`), and calls `deploy` ([`UniswapV3PoolDeployer.sol:27-37`](v3-core/contracts/UniswapV3PoolDeployer.sol#L27-L37)):
...
The pool's constructor reads its config *back from the deployer* ([`UniswapV3Pool.sol:117-123`](v3-core/contracts/UniswapV3Pool.sol#L117-L123): `IUniswapV3PoolDeployer(msg.sender).parameters()`) instead of taking constructor arguments. Constructor args are part of the init code, and the init code must be constant for the CREATE2 address to be predictable by `PoolAddress.computeAddress` ([`v3-periphery/contracts/libraries/PoolAddress.sol:33-47`](v3-periphery/contracts/libraries/PoolAddress.sol#L33-L47)) using the fixed `POOL_INIT_CODE_HASH` (`:6`).

### 2.3 Pool storage

[`v3-core/contracts/UniswapV3Pool.sol:42-99`](v3-core/contracts/UniswapV3Pool.sol#L42-L99)
...
| `ticks[int24]` | `Tick.Info` | per initialized tick: `liquidityGross`, `liquidityNet`, `feeGrowthOutside0/1`, oracle "outside" snapshots, `initialized` ([`Tick.sol:17-37`](v3-core/contracts/libraries/Tick.sol#L17-L37)) |
| `tickBitmap[int16]` | uint256 | 1 bit per (tick / tickSpacing): is it initialized |
| `positions[bytes32]` | `Position.Info` | keyed by `keccak256(owner, tickLower, tickUpper)`: `liquidity`, `feeGrowthInside0/1Last`, `tokensOwed0/1` ([`Position.sol:13-22`](v3-core/contracts/libraries/Position.sol#L13-L22)) |
...
**`initialize(sqrtPriceX96)`** ([`UniswapV3Pool.sol:271-289`](v3-core/contracts/UniswapV3Pool.sol#L271-L289)): can be called once (`require(slot0.sqrtPriceX96 == 0)`), derives the tick, writes observation `[0]`, sets `unlocked = true`. Not guarded by `lock` because the lock is not yet set. Anyone can call it; the caller picks the initial price, so the first LP should check it.
...
The pool computes how many tokens the liquidity is worth, then asks `msg.sender` to *pay* via a callback, then verifies by balance diff. There is no `approve` to the pool; the router's callback ([`v3-periphery/contracts/base/LiquidityManagement.sol:25-35`](v3-periphery/contracts/base/LiquidityManagement.sol#L25-L35)) does `transferFrom(payer, pool)` after checking `msg.sender` is the genuine pool via `CallbackValidation.verifyCallback` ([`libraries/CallbackValidation.sol:28-35`](v3-periphery/contracts/libraries/CallbackValidation.sol#L28-L35)). Any contract can integrate the pool directly by implementing the callback.
...
The three branches are the geometry from 2.1. Below the range you only owe token0 (the pool would need it as price rises through your range); above it only token1; inside it, both, split at the current price. Only the "inside" case touches the global `liquidity` and writes an oracle observation, because only in-range liquidity affects swaps. The signed helpers at [`SqrtPriceMath.sol:201-226`](v3-core/contracts/libraries/SqrtPriceMath.sol#L201-L226) round *up* when adding liquidity and *down* when removing, always in the pool's favour.

**`_updatePosition(...)`** (`:379-453`) does the bookkeeping:

1. Fetch the position slot (`Position.get`, keccak of owner+ticks, [`Position.sol:30-37`](v3-core/contracts/libraries/Position.sol#L30-L37)).
2. If `liquidityDelta != 0`: get the current oracle accumulators (`observeSingle(..., 0, ...)`, `:396-404`), then `ticks.update` for the lower tick with `upper = false` and the upper tick with `upper = true` (`:406-429`). `Tick.update` ([`Tick.sol:110-150`](v3-core/contracts/libraries/Tick.sol#L110-L150)) adds `liquidityDelta` to `liquidityGross`, adds it to `liquidityNet` for the lower tick and *subtracts* it for the upper tick (`:147-149`), checks `maxLiquidityPerTick`, and, if the tick was just initialised and is at or below the current tick, snapshots the global fee growth as `feeGrowthOutside` (`:132-142`). It returns `flipped` (initialised <-> uninitialised).
3. If flipped, toggle the bit in `tickBitmap.flipTick` ([`TickBitmap.sol:23-32`](v3-core/contracts/libraries/TickBitmap.sol#L23-L32)).
4. Compute `feeGrowthInside` for the range (`Tick.getFeeGrowthInside`, next section) and call `position.update` ([`Position.sol:44-87`](v3-core/contracts/libraries/Position.sol#L44-L87)), which credits fees to `tokensOwed`.
...
Paying each LP on every swap is impossible. Instead, every swap adds `feeAmount * 2^128 / liquidity` to `feeGrowthGlobal{0,1}X128` ([`UniswapV3Pool.sol:689-690`](v3-core/contracts/UniswapV3Pool.sol#L689-L690)). "Growth" is *fees per unit of liquidity*, in Q128 to keep precision. An LP with `L` units who has been in range while the growth went from `g_then` to `g_now` is owed `L * (g_now − g_then) / 2^128`. That is exactly `Position.update` ([`Position.sol:61-76`](v3-core/contracts/libraries/Position.sol#L61-L76)):
...
But a position only earns while the price is *inside* its range. So each position needs `feeGrowthInside(tickLower, tickUpper)`, and every tick stores `feeGrowthOutside`: the growth that happened on the *other* side of the tick relative to the current price. `Tick.getFeeGrowthInside` ([`Tick.sol:60-95`](v3-core/contracts/libraries/Tick.sol#L60-L95)):
...
The subtraction trick: `feeGrowthOutside` is never "true" fees below/above the tick in absolute terms. It is initialised to `feeGrowthGlobal` if the tick is at or below the current price, or 0 otherwise ([`Tick.sol:132-140`](v3-core/contracts/libraries/Tick.sol#L132-L140)), and every time the price crosses the tick it is flipped with `feeGrowthOutside = feeGrowthGlobal − feeGrowthOutside` (`Tick.cross`, [`Tick.sol:178-179`](v3-core/contracts/libraries/Tick.sol#L178-L179)). Because positions only ever look at *differences* of `feeGrowthInside` between two moments, the arbitrary constant cancels out, and `uint256` underflow during these subtractions is intentional and harmless. This is why V3 fee math "just works" with two SSTOREs per crossed tick regardless of how many positions share it.

### 2.6 `swap`: the loop

[`v3-core/contracts/UniswapV3Pool.sol:596-788`](v3-core/contracts/UniswapV3Pool.sol#L596-L788)
...
Like V2, the output goes out optimistically, the caller is asked to pay through a callback, and the pool verifies by balance. Unlike V2 there is no `k` check: the amounts were computed exactly, so the pool just checks it received `amount0`. The caller can use the output tokens inside the callback to source the input (a flash swap). `sqrtPriceLimitX96` is both a slippage guard and a "partial fill" tool: the swap stops at the limit and returns smaller amounts rather than reverting (which is why `SwapRouter.exactOutputInternal` re-checks `amountOutReceived == amountOut` when no limit was passed, [`SwapRouter.sol:199`](v3-periphery/contracts/SwapRouter.sol#L199)).

### 2.7 `burn` and `collect`

`burn(tickLower, tickUpper, amount)` ([`UniswapV3Pool.sol:517-543`](v3-core/contracts/UniswapV3Pool.sol#L517-L543)) calls `_modifyPosition` with a *negative* delta, which returns negative amounts. It **does not transfer anything**; it adds the freed principal to `position.tokensOwed0/1` (`:535-540`). `burn(..., 0)` is a legal "poke" that just refreshes fees owed (allowed by `Position.update` only if the position has liquidity, [`Position.sol:53-55`](v3-core/contracts/libraries/Position.sol#L53-L55)).
...
- `mint(params)` (`:128-182`): `addLiquidity` ([`base/LiquidityManagement.sol:51-89`](v3-periphery/contracts/base/LiquidityManagement.sol#L51-L89)) computes `L` from the desired token amounts (`LiquidityAmounts.getLiquidityForAmounts`, `:71-77`), calls `pool.mint(address(this), ...)` with `MintCallbackData{poolKey, payer: msg.sender}`, and enforces `amount0Min/amount1Min` (`:88`). Back in `mint` it `_mint`s the ERC-721, reads the pool's `feeGrowthInsideLast` for the aggregated position (`:158-159`), and stores the per-token snapshot.
...
**SwapRouter** (`v3-periphery/contracts/SwapRouter.sol`). Routes are packed bytes `tokenIn(20) | fee(3) | tokenOut(20) | fee(3) | ...` decoded by `Path` ([`libraries/Path.sol:42-68`](v3-periphery/contracts/libraries/Path.sol#L42-L68)).
...
- `exactInput` (`:132-166`) loops: hop 1 pays from `msg.sender` and sends output to the router; every subsequent hop has `payer = address(this)` (`:157`) and the callback pays from the router's own balance (`PeripheryPayments.pay`, [`base/PeripheryPayments.sol:52-69`](v3-periphery/contracts/base/PeripheryPayments.sol#L52-L69), branch `payer == address(this)`).
...
V3 deploys one contract per pool and moves ERC-20s on every action. V4 (`v4-core/src/PoolManager.sol`) keeps **all pools in one contract**: `mapping(PoolId id => Pool.State) internal _pools` (`:93`). A pool is identified by a `PoolKey` ([`types/PoolKey.sol:11-22`](v4-core/src/types/PoolKey.sol#L11-L22): `currency0`, `currency1`, `fee`, `tickSpacing`, `hooks`) and its id is `keccak256(abi.encode(key))` ([`types/PoolId.sol:11-16`](v4-core/src/types/PoolId.sol#L11-L16)). Keys are *not* stored; the caller must pass the full key every time. `Currency` is an address wrapper where `address(0)` means native ETH ([`types/Currency.sol:7`](v4-core/src/types/Currency.sol#L7), `:38`), so ETH pools need no WETH.

The V3 pool contract became a *library*, `libraries/Pool.sol`, operating on `Pool.State` (`:83-91`): `slot0` (packed into a single `bytes32`, [`types/Slot0.sol:8-9`](v4-core/src/types/Slot0.sol#L8-L9): `sqrtPriceX96 | tick | protocolFee | lpFee`), `feeGrowthGlobal0/1`, `liquidity`, `ticks`, `tickBitmap`, `positions`. Note what is gone: no oracle (`observations`), no per-tick oracle snapshots, no `tokensOwed` in positions ([`libraries/Position.sol:19-25`](v4-core/src/libraries/Position.sol#L19-L25)), no `protocolFees` per pool. Those moved to hooks or to the manager.

**`unlock` and flash accounting.** Nothing except `initialize` can be called directly. A caller must first call `unlock(data)` ([`PoolManager.sol:104-114`](v4-core/src/PoolManager.sol#L104-L114)):
...
- `CurrencyDelta` ([`libraries/CurrencyDelta.sol:10-41`](v4-core/src/libraries/CurrencyDelta.sol#L10-L41)): slot `keccak256(target, currency)`; `applyDelta` returns previous and next values.
- `NonzeroDeltaCount` (`libraries/NonzeroDeltaCount.sol`): a transient counter of how many `(address, currency)` pairs currently have a non-zero delta. `_accountDelta` ([`PoolManager.sol:368-378`](v4-core/src/PoolManager.sol#L368-L378)) increments it when a delta goes from 0 to non-zero and decrements on the way back.
...
|---|---|---|---|
| `sync(currency)` | `:279-288` | none; records `balanceOf(manager)` in transient `CurrencyReserves` ([`libraries/CurrencyReserves.sol:27-32`](v4-core/src/libraries/CurrencyReserves.sol#L27-L32)) | no |
...
**Reading state.** Since all pools share one contract, there are no per-pool getters. `Extsload` ([`Extsload.sol:10`](v4-core/src/Extsload.sol#L10)) and `Exttload` ([`Exttload.sol:10`](v4-core/src/Exttload.sol#L10)) expose raw `sload`/`tload` of arbitrary slots; `StateLibrary` ([`libraries/StateLibrary.sol:40-63`](v4-core/src/libraries/StateLibrary.sol#L40-L63) for `getSlot0`) knows the layout (`_pools` is slot 6, `:11`, and the offsets of each field inside `Pool.State`, `:14-28`) and decodes it.

### 3.2 Hooks

A pool's `hooks` address is a contract implementing `IHooks` (`interfaces/IHooks.sol`). The manager calls it at up to ten points: `before/afterInitialize`, `before/afterAddLiquidity`, `before/afterRemoveLiquidity`, `before/afterSwap`, `before/afterDonate`. Every hook function must return its own selector (`Hooks.callHook`, [`libraries/Hooks.sol:131-155`](v4-core/src/libraries/Hooks.sol#L131-L155), check at `:152`), and failures are wrapped in `HookCallFailed` with the original revert bubbled (`:137`).

**Permissions live in the address.** [`Hooks.sol:27-47`](v4-core/src/libraries/Hooks.sol#L27-L47) defines 14 flag bits; the manager checks `uint160(address(hook)) & FLAG != 0` (`hasPermission`, `:337-339`). A hook that wants `beforeSwap` must be deployed (via CREATE2 salt mining) to an address whose bit 7 is set. This makes the permission check a pure bit test with no storage read, and makes the hook's capabilities visible from its address. `validateHookPermissions` (`:83-103`) is meant for hook constructors; `isValidHookAddress` (`:109-127`) is enforced at `initialize` ([`PoolManager.sol:126`](v4-core/src/PoolManager.sol#L126)): a return-delta flag requires the matching action flag, and a non-zero hook address must have at least one flag or a dynamic fee.

**Which calls happen where** (`Hooks.sol`):

- `beforeInitialize` / `afterInitialize` (`:178-192`) around `Pool.initialize`.
- `beforeModifyLiquidity` (`:195-206`) picks `beforeAddLiquidity` when `liquidityDelta > 0`, else `beforeRemoveLiquidity`. `afterModifyLiquidity` (`:209-245`) likewise, and if the hook has the `RETURNS_DELTA` flag it may return a `BalanceDelta` that is *charged to the caller* (`callerDelta = callerDelta − hookDelta`, `:230`/`:242`) and credited to the hook ([`PoolManager.sol:181`](v4-core/src/PoolManager.sol#L181)).
...
**Fees.** `PoolKey.fee` is either a static LP fee in pips (max `1_000_000` = 100%, [`LPFeeLibrary.sol:25`](v4-core/src/libraries/LPFeeLibrary.sol#L25)) or the sentinel `DYNAMIC_FEE_FLAG = 0x800000` (`:15`). Dynamic pools start at 0 (`getInitialLPFee`, `:51-56`) and the hook sets the fee via `updateDynamicLPFee` ([`PoolManager.sol:339-346`](v4-core/src/PoolManager.sol#L339-L346), hook-only) or per-swap by returning a fee with `OVERRIDE_FEE_FLAG = 0x400000` (`:19`) from `beforeSwap` ([`Pool.sol:303-305`](v4-core/src/libraries/Pool.sol#L303-L305)). Protocol fees (`ProtocolFees.sol`) are set per pool by the `protocolFeeController` (`:35-41`), capped at 0.1% per direction ([`ProtocolFeeLibrary.sol:8`](v4-core/src/libraries/ProtocolFeeLibrary.sol#L8)), stored as two 12-bit fields in `slot0`, and accrued per currency in `protocolFeesAccrued` (`:21`). The combined swap fee is `protocol + lp − protocol*lp/1e6` (`ProtocolFeeLibrary.calculateSwapFee`, `:38-46`): protocol first, LP on the remainder.

### 3.3 What moved where in `Pool.sol`

| V3 `UniswapV3Pool` | V4 | Notes |
|---|---|---|
| `initialize` (`:271`) | `Pool.initialize` ([`Pool.sol:100-107`](v4-core/src/libraries/Pool.sol#L100-L107)) via `PoolManager.initialize` (`:117-142`) | also validates hook address, tick spacing bounds ([`TickMath.sol:26-28`](v3-core/contracts/libraries/TickMath.sol#L26-L28)), stores the lp fee in slot0 |
| `mint` + `burn` + `_modifyPosition` + `_updatePosition` | `Pool.modifyLiquidity` ([`Pool.sol:146-238`](v4-core/src/libraries/Pool.sol#L146-L238)) via `PoolManager.modifyLiquidity` (`:145-184`) | one signed `liquidityDelta`; returns `(principalDelta, feesAccrued)`; positions also keyed by a `salt` ([`Position.sol:48-67`](v4-core/src/libraries/Position.sol#L48-L67)) so one owner can hold many positions in the same range |
| `collect` / `tokensOwed` | gone | `Position.update` ([`Position.sol:76-102`](v4-core/src/libraries/Position.sol#L76-L102)) *returns* fees owed and they are immediately accounted as a positive delta to the caller ([`PoolManager.sol:171`](v4-core/src/PoolManager.sol#L171)). Any `modifyLiquidity(0)` poke collects. |
| `swap` (`:596`) | `Pool.swap` ([`Pool.sol:279-463`](v4-core/src/libraries/Pool.sol#L279-L463)) via `PoolManager.swap` (`:187-227`) | same loop; `amountSpecified < 0` means exact **input** ([`types/PoolOperation.sol:22-23`](v4-core/src/types/PoolOperation.sol#L22-L23), [`SwapMath.sol:61`](v3-core/contracts/libraries/SwapMath.sol#L61)), the opposite of V3's sign; `lpFeeOverride` from the hook; protocol fee taken from `amountIn + feeAmount` ([`Pool.sol:391-393`](v4-core/src/libraries/Pool.sol#L391-L393)); returns a packed `BalanceDelta` ([`types/BalanceDelta.sol:14-18`](v4-core/src/types/BalanceDelta.sol#L14-L18), two int128s in one int256) |
...
`SwapMath.getSqrtPriceTarget` ([`v4-core/src/libraries/SwapMath.sol:20-37`](v4-core/src/libraries/SwapMath.sol#L20-L37)) is the branch-free version of V3's ternary at [`UniswapV3Pool.sol:665-667`](v3-core/contracts/UniswapV3Pool.sol#L665-L667).
...
- **Rounding direction.** V3 rounds every amount the pool *receives* up and every amount it *pays* down ([`SqrtPriceMath.sol:201-226`](v3-core/contracts/libraries/SqrtPriceMath.sol#L201-L226), `SwapMath.sol` `roundUp` flags). `getAmountIn` in V2 adds `+1` ([`UniswapV2Library.sol:58`](v2-periphery/contracts/libraries/UniswapV2Library.sol#L58)). Reversing any of these gives a "wei-by-wei" drain. If you write an integration that computes amounts yourself, copy the rounding.
...
- **First-depositor / inflation attack.** V2 burns `MINIMUM_LIQUIDITY` ([`UniswapV2Pair.sol:121`](v2-core/contracts/UniswapV2Pair.sol#L121)). Vaults that copy the "shares = amount * supply / balance" pattern without an equivalent are vulnerable to a donate-then-round-down attack.
...
- **`uint112` overflow in V2.** `_update` reverts if a balance exceeds `2^112 − 1` ([`UniswapV2Pair.sol:74`](v2-core/contracts/UniswapV2Pair.sol#L74)). Someone can brick a pair by donating; `skim` is the recovery.
- **Tick spacing and `maxLiquidityPerTick`.** `enableFeeAmount` caps `tickSpacing < 16384` ([`UniswapV3Factory.sol:67`](v3-core/contracts/UniswapV3Factory.sol#L67)) because `nextInitializedTickWithinOneWord` multiplies compressed ticks back by spacing and would overflow `int24`. `Tick.update` enforces `maxLiquidityPerTick` ([`Tick.sol:128`](v3-core/contracts/libraries/Tick.sol#L128)) so `liquidityGross` cannot overflow `uint128` when summed across ticks.
- **`sqrtPriceLimitX96` and sandwiches.** Passing `MIN/MAX_SQRT_RATIO ± 1` (the router default) means "any price"; the only protection is `amountOutMinimum`. A tight limit gives a partial fill instead of a revert, which some integrations mishandle ([`SwapRouter.sol:199`](v3-periphery/contracts/SwapRouter.sol#L199) guards the exact-output case).
- **V3 tick/price lag.** After a leftward cross `slot0.tick` is `tickNext − 1` while `sqrtPriceX96` sits exactly on `tickNext` ([`UniswapV3Pool.sol:725`](v3-core/contracts/UniswapV3Pool.sol#L725), and the V4 comment at [`Pool.sol:409-412`](v4-core/src/libraries/Pool.sol#L409-L412)). Code that recomputes the tick from the price and compares will see a mismatch of one; `donate` in V4 must check both.
...
- **V4 `settle` ordering.** Calling `settle` without a preceding `sync` for an ERC-20 attributes the wrong reserve snapshot; `collectProtocolFees` refuses to run while a currency is synced ([`ProtocolFees.sol:49-52`](v4-core/src/ProtocolFees.sol#L49-L52)) precisely to stop balance-based accounting from being confused. Native ETH `settle` needs `msg.value`, and a non-zero `msg.value` with an ERC-20 synced reverts ([`PoolManager.sol:356`](v4-core/src/PoolManager.sol#L356)).
- **`feeGrowthGlobal` can be inflated by `donate`** in a V4 pool with a single position ([`Pool.sol:80-82`](v4-core/src/libraries/Pool.sol#L80-L82)); do not use it as a metric across pools.

---

## 6. Exercises to trace yourself

1. **V2 fee proof.** Open [`v2-core/contracts/UniswapV2Pair.sol:176-182`](v2-core/contracts/UniswapV2Pair.sol#L176-L182). With `reserve0 = 1000`, `reserve1 = 1000`, `amount0Out = 0`, `amount1Out = 90`, work out by hand the minimum `balance0` that passes the k-check, then confirm it equals `getAmountIn(90, 1000, 1000)` from [`UniswapV2Library.sol:53-59`](v2-periphery/contracts/libraries/UniswapV2Library.sol#L53-L59).
2. **V2 protocol fee.** Set `feeTo`, do two swaps, then call `mint`. Follow `_mintFee` at [`UniswapV2Pair.sol:89-107`](v2-core/contracts/UniswapV2Pair.sol#L89-L107) and verify that the shares minted to `feeTo` are worth exactly 1/6 of the `√k` growth by computing the pool value before and after.
3. **V3 tick bitmap.** With `tickSpacing = 60` and initialised ticks at `-120`, `0`, `600`, hand-trace `TickBitmap.nextInitializedTickWithinOneWord(tick = 30, 60, lte = true)` and `(tick = 30, 60, lte = false)` at [`TickBitmap.sol:42-77`](v3-core/contracts/libraries/TickBitmap.sol#L42-L77), including the `compressed` rounding at `:48-49`. Then do `tick = -1` and see why the extra `compressed--` exists.
4. **V3 fee growth invariance.** Take a position `[Pa, Pb]` and a price path that starts below `Pa`, goes above `Pb`, then comes back inside. Track `feeGrowthOutside` of both ticks through `Tick.update` ([`Tick.sol:132-140`](v3-core/contracts/libraries/Tick.sol#L132-L140)) and each `Tick.cross` (`:178-179`), then compute `getFeeGrowthInside` (`:60-95`) before and after and convince yourself the difference equals only the fees earned while inside the range.
5. **V3 swap step.** Start from [`UniswapV3Pool.sol:663-671`](v3-core/contracts/UniswapV3Pool.sol#L663-L671) and step into `SwapMath.computeSwapStep` ([`SwapMath.sol:21-97`](v4-core/src/libraries/SwapMath.sol#L21-L97)) for an exact-*output* swap (`amountRemaining < 0`) that hits the price target. Identify which of `amountIn`/`amountOut` is recomputed at `:70-84` and why `feeAmount` uses `1e6 − feePips` as the denominator.
6. **V3 exact-output multi-hop.** Trace `SwapRouter.exactOutput` ([`SwapRouter.sol:224-243`](v3-periphery/contracts/SwapRouter.sol#L224-L243)) for a 2-pool path. Draw the call stack at the moment the innermost `pay` runs and explain how `amountInCached` (`:38`, `:79`, `:240`) carries the answer out through the nested callbacks.
7. **V4 delta netting.** Write on paper the sequence of `_accountDelta` calls ([`PoolManager.sol:368-378`](v4-core/src/PoolManager.sol#L368-L378)) and the value of `NonzeroDeltaCount` for a router that does two swaps A->B then B->C inside one `unlockCallback` and only `settle`s A and `take`s C. Explain why no B transfer ever happens.
8. **V4 hook delta.** A `beforeSwap` hook returns `BeforeSwapDelta(specified = +amountIn, unspecified = −X)` on an exact-input swap. Follow `Hooks.beforeSwap` ([`Hooks.sol:248-282`](v4-core/src/libraries/Hooks.sol#L248-L282)) and `Hooks.afterSwap` (`:285-315`), then `PoolManager.swap` (`:187-227`), and work out (a) what amount `Pool.swap` receives, (b) what the trader's final delta is, (c) what the hook must do before `unlock` returns.
9. **V4 address mining.** Using [`Hooks.sol:27-47`](v4-core/src/libraries/Hooks.sol#L27-L47), compute the lowest 14 bits an address needs to get `beforeSwap`, `afterSwap` and `beforeSwapReturnDelta`, and check `isValidHookAddress` (`:109-127`) would accept it with a static fee and with `DYNAMIC_FEE_FLAG`.
